...
export default class Renderer extends React.Component {
    ...
    onDocumentMouseDown = (e) => {
        ...
        if (intersects.length > 0) {
            ...
            if (this.props.crewMemberSelected === 1) {
                if ((!this.props.startHandrail || !this.props.endHandrail) &&
                    ...
                    this.props.handleStartEndHandrailsChanged(this.props.startHandrail ? 'end' : 'start', intersects[0].object, true, this.props.crewMemberSelected);
                }
                ...
                // Else if the chosen handrail is the preselected start handrail, clear the start handrail.
                    ...
                    this.props.handleStartEndHandrailsChanged('start', null, false, this.props.crewMemberSelected);
                }
                ...
                // Else if the chosen handrail is the preselected end handrail, clear the end handrail.
                    ...
                    this.props.handleStartEndHandrailsChanged('end', null, false, this.props.crewMemberSelected);
                }
            }
            if (this.props.crewMemberSelected === 2) {
                // If the start handrailsecond or the end handrail equals null (meaning neither handrail has been chosen)
                // or if the chosen handrail is not the preselected start or end handrail, mark the handrail respectively.
                if ((!this.props.startHandrailSecond || !this.props.endHandrailSecond) &&
                    this.props.startHandrailSecond != intersects[0].object.name.replace('.stl', '') &&
                    this.props.endHandrailSecond != intersects[0].object.name.replace('.stl', '')) {
                    //debugger;
                    this.props.handleStartEndHandrailsChanged(this.props.startHandrailSecond ? 'end' : 'start', intersects[0].object, true, this.props.crewMemberSelected);
                } else if (this.props.startHandrailSecond == intersects[0].object.name.replace('.stl', '')) {
                    this.props.handleStartEndHandrailsChanged('start', null, false, this.props.crewMemberSelected);
                } else if (this.props.endHandrailSecond == intersects[0].object.name.replace('.stl', '')) {
                    this.props.handleStartEndHandrailsChanged('end', null, false, this.props.crewMemberSelected);
                }
            }
        }
    ...
    `;
        ...
        if (handrailFiles && Object.keys(handrailFiles).length > 0 && strFiles && strFiles.length > 0) {
            ...
            Object.entries(handrailFiles).forEach(([name, handrailFile]) => {
                let color = hrColor;
                let scale = 1;
                var handrailMesh = loadMeshFromFile(handrailFile, {
                    color
                }, {
                    scale
                });
                ...
                //PHASE 3 MOD Tenadam Weldesemayat/tweldesemayat@student.umuc.edu 8/10/2018 - update for accounting two crew members
                ...
                if (this.props.crewMemberSelected === 1) {
                    if (name === startHandrail + '.stl') {
                        ...
                        // create handrail
                        handrailMesh = loadMeshFromFile(handrailFile, {
                            color
                        }, {
                            scale
                        });
                ...
                // set start/end/route handrail color for crew member 2
                if (this.props.crewMemberSelected === 2) {
                    if (name === startHandrailSecond + '.stl') {
                        //set color of handrail
                        color = hrStartColor;
                        ...
                        //add handrail clone to scene at handrailMesh location
                        handrailMesh.add(handrailMeshClone);
                    } else if (name === endHandrailSecond + '.stl') {
                        // set color of handrail
                        color = hrEndColor;
...
Renderer.propTypes = {
    ...
    routesSecond: PropTypes.array,
    wingspan: PropTypes.number,
    wingspanSecond: PropTypes.number,
};